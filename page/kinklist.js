...
$(function(){
    ...
    function loadListText() {
        var list = $("#listType").val();
        var lang = $('#langSelect').val() || '';
        var fileToRead = (lang ? (lang + '/') : '') + list + '.txt';

        $.get(fileToRead, function(data) {
            $('#Kinks').val(data.replace(/^\uFEFF/, ''));
            ...
            if(fallback === fileToRead) return;
            $.get(fallback, function(data) {
                $('#Kinks').val(data.replace(/^\uFEFF/, ''));
                var selection = inputKinks.saveSelection();
                var kinksText = $('#Kinks').val();
                kinks = inputKinks.parseKinksText(kinksText);
    ...
    $("#listType").change(loadListText);
    $('#langSelect').change(loadListText);
    ...
    inputKinks = {
        ...
        parseKinksText: function(kinksText){
            ...
            for(var i = 0; i < lines.length; i++){
                ...
                if(line[0] === '(' || line[0] === '（') {
                    var fieldLine = line;
                    if(fieldLine[fieldLine.length - 1] === ')' || fieldLine[fieldLine.length - 1] === '）') {
                        fieldLine = fieldLine.substring(1, fieldLine.length - 1);
                    }
                    else {
    ...
    // Attempt to load localized file on startup, fall back to embedded textarea
    try {
        loadListText();
    }
    catch(e) {
